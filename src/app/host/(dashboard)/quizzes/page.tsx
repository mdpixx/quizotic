...
function statusStyle(tone: 'ready' | 'draft' | 'attention') {
  ...
  if (tone === 'attention') return { background: '#FFFBEB', color: 'var(--color-warning)', border: '1px solid #FDE68A' }
...
const SCHEDULED_CHIP_STYLE = { background: '#FFFBEB', color: 'var(--color-warning)', border: '1px solid #FDE68A' }
...
export default function QuizzesPage() {
  ...
  return (
    ...
    <div className="p-6 md:p-8 max-w-[1280px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>
            <span className="font-display font-bold" style={{ color: 'var(--color-ink)' }}>{quizzes.length}</span>
            {' '}quiz{quizzes.length !== 1 ? 'zes' : ''} saved{subjects.length > 0 ? ` · ${subjects.length} subject${subjects.length === 1 ? '' : 's'}` : ''}
          </p>
        </div>
      ...
      {error && (
        <div className="mb-4 px-4 py-3 rounded-xl text-sm font-medium" style={{ background: '#FEE2E2', color: 'var(--color-danger)' }}>
          {error}
        </div>
      )}

      <div className="mb-5 grid md:grid-cols-2 gap-3">
        <div className="rounded-[14px] bg-white border p-4 flex items-center gap-3" style={{ borderColor: 'var(--color-line)', boxShadow: '0 1px 2px rgba(15,27,61,0.04)' }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'var(--color-ink)', color: 'var(--color-yellow)' }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><path d="M8 5v14l11-7z"/></svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-display text-[13px] font-black" style={{ color: 'var(--color-ink)' }}>Host live</p>
            <p className="text-[11px]" style={{ color: 'var(--color-text-muted)' }}>Run a room with a join code, timer, and leaderboard.</p>
          </div>
        </div>
        <div className="rounded-[14px] bg-white border p-4 flex items-center gap-3" style={{ borderColor: 'var(--color-line)', boxShadow: '0 1px 2px rgba(15,27,61,0.04)' }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#ECFDF5', color: 'var(--color-success)' }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="font-display text-[13px] font-black" style={{ color: 'var(--color-ink)' }}>Share self-paced</p>
      ...
      <div className="mb-6 rounded-[14px] px-3 py-2.5 flex items-center gap-3 flex-wrap" style={{ background: '#fff', border: '1px solid var(--color-line)', boxShadow: '0 1px 2px rgba(15,27,61,0.03)' }}>
        <div className="relative flex-1 min-w-[200px] max-w-[360px]">
          <input
            ...
            style={{ background: 'var(--color-paper-2)', border: '1px solid var(--color-line)', color: 'var(--color-ink)' }}
          />
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {subjects.slice(0, 5).map(sub => (
              <button
                ...
                className="chip"
                style={{ background: activeSubject === sub ? 'var(--color-ink)' : 'var(--color-paper)', color: activeSubject === sub ? '#fff' : 'var(--color-text-secondary)', cursor: 'pointer', border: '1px solid var(--color-line)' }}
              >
                {sub}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center rounded-[16px]" style={{ background: 'var(--color-paper-2)', border: '1px dashed #DDD4BC' }}>
          ...
          <p className="font-display text-[18px] font-black mb-1" style={{ color: 'var(--color-ink)' }}>
        ...
        <>
          ...
          {view === 'grid' && recent.length > 0 && (
            <section className="mb-8">
              <div className="flex items-end justify-between mb-3">
                <div>
                  <h2 className="text-[16px] font-black" style={{ color: 'var(--color-ink)' }}>Recent</h2>
                      ...
                      <div className="relative aspect-[5/3] overflow-hidden" style={{ background: gradientFor(quiz.id) }}>
                        ...
                        <div className="relative z-10 h-full p-3 flex flex-col justify-between">
                          <div className="flex items-start justify-between">
                            <span className="chip" style={{ background: 'rgba(255,255,255,0.86)', color: 'var(--color-ink)', backdropFilter: 'blur(6px)' }}>
                          ...
                          <h3 className="text-[15px] font-black leading-snug line-clamp-2" style={{ fontFamily: 'var(--font-display)', color: '#fff', textShadow: '0 1px 3px rgba(0,0,0,.35)' }}>
                            {quiz.title}
                          </h3>
                        </div>
                      </div>
                        ...
                        {quiz.asyncShareSlug && (
                          <div className="mb-2 rounded-lg px-2.5 py-2" style={{ background: quiz.asyncNeedsRepublish ? '#FFFBEB' : '#F0FDF4', border: `1px solid ${quiz.asyncNeedsRepublish ? '#FDE68A' : '#BBF7D0'}` }}>
                            <div className="flex items-center justify-between">
                              ...
                              <Link href={`/host/quizzes/${quiz.id}/report`} className="text-[10px] font-bold underline" style={{ color: 'var(--color-text-muted)' }}>
                                Results
                              </Link>
                            </div>
                        ...
                        <div className="flex gap-2 mt-auto items-center">
                          ...
                          >
                            {startingId === quiz.id ? (
                              ...
                              <>
                                <span className="play-dot" style={{ width: '16px', height: '16px' }}><svg viewBox="0 0 24 24" fill="var(--color-ink)" className="w-2 h-2"><path d="M8 5v14l11-7z"/></svg></span>
                                Host live
                              </>
                            )}
                          </button>
          ...
          {(view === 'list' ? filtered : rest).length > 0 && (
            <section>
              <div className="flex items-end justify-between mb-3">
                <div>
                  <h2 className="text-[16px] font-black" style={{ color: 'var(--color-ink)' }}>{view === 'list' ? 'All quizzes' : 'Recently edited'}</h2>
      ...
      <AnimatePresence>
        {confirmDelete && (
          ...
          >
            ...
            >
              <h3 className="text-lg font-black mb-2" style={{ color: 'var(--color-ink)' }}>Delete Quiz?</h3>
