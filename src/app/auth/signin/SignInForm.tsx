...
// Remember the last email a user signed in with, so a returning visitor sees
// it pre-filled instead of re-typing it. Stored client-side only.
const LAST_EMAIL_KEY = 'quizotic:lastEmail'
...
export default function SignInForm({ intent, callbackUrl = '/host' }: SignInFormProps) {
  ...
  // Pre-fill the returning user's email straight from localStorage at init.
  // Signup intent starts blank — a new user shouldn't see an old address.
  // Guarded for SSR (no window) and private-mode (localStorage can throw).
      ...
      <div className="max-w-sm w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-base" style={{ color: '#94A3B8' }}>
            {isSignup ? 'Start hosting quizzes free' : 'Sign in to your account'}
          </p>
        </div>
