  ...
  return (
    <Link
      ...
      style={{
        ...
        color: active ? '#4361EE' : '#4A5568',
      }}
    >
      <span className="text-base leading-none">{item.icon}</span>
      <span>{item.label}</span>
      {active && (
        <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: '#4361EE' }} />
      )}
    </Link>
  )
}
...
function SidebarContent({ onNavClick }: { onNavClick?: () => void }) {
  ...
  return (
    <>
      {/* Logo */}
      <div className="px-5 py-5 border-b" style={{ borderColor: 'rgba(67,97,238,0.08)' }}>
      ...
      <div className="px-4 pt-4 pb-2 flex gap-2">
        ...
        <Link
          ...
          onClick={onNavClick}
          className="flex-1 text-center text-xs font-bold py-2 rounded-xl transition-all hover:scale-[1.02] text-white"
          style={{ background: SLIDES_GRADIENT }}
        >
          + Slides
        </Link>
      </div>
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        <p className="px-3 pt-1 pb-1 text-[10px] font-black uppercase tracking-widest" style={{ color: '#C7D7FD' }}>
          Main
        </p>
        {navItems.map((item) => (
        ...
        <div className="pt-3 mt-3 border-t" style={{ borderColor: 'rgba(67,97,238,0.08)' }}>
          <p className="px-3 pb-1 text-[10px] font-black uppercase tracking-widest" style={{ color: '#C7D7FD' }}>
            Tools
          </p>
          <Link
            href="/host/billing"
          ...
          {isAdmin && (
            ...
            >
              ...
              <span className="ml-auto text-[9px] font-black px-1.5 py-0.5 rounded-full" style={{ background: '#7C3AED', color: '#fff' }}>
                ADMIN
              </span>
            </Link>
          )}
      ...
      {/* User profile */}
      {user && (
        <div className="px-3 pb-4 border-t pt-3" style={{ borderColor: 'rgba(67,97,238,0.08)' }}>
          <div ref={menuRef} className="relative">
            <button
              onClick={() => setUserMenuOpen((o) => !o)}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all hover:bg-blue-50"
            >
              {user.image ? (
                <img
                  src={user.image}
              ...
              {userMenuOpen && (
                ...
                >
                  <div className="px-4 py-2.5 border-b" style={{ borderColor: '#F1F5F9' }}>
                    <p className="text-xs font-bold truncate" style={{ color: '#1B2559' }}>
                      {user.name}
                    </p>
                    <p className="text-xs truncate" style={{ color: '#9CA3AF' }}>
                      {user.email}
